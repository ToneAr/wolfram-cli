Function[
	...
	Module[
		{stream, held, pending = {}, result = "", processResult, runCommand, splitCompoundExpressions, promptedInput, promptedInputString},
		...
		runCommand[command_String] := RunProcess[
		...
		splitCompoundExpressions[expression_] := If[
			MatchQ[expression, HoldComplete[CompoundExpression[___]]],
			Flatten[splitCompoundExpressions /@ (
				Map[HoldComplete, expression, {2}] /.
					HoldComplete[CompoundExpression[expressions___]] :> {expressions}
			)],
			{expression}
		];
			...
			Block[{
				...
				While[
					...
					If[MatchQ[held, HoldComplete[Run[_String]]],
						processResult = ReleaseHold[
							held /. HoldComplete[Run[command_String]] :>
								HoldComplete[runCommand[command]]
						];
						If[processResult["StandardOutput"] =!= "",
							Print[StringTrim[processResult["StandardOutput"], "\n"]]
						];
						Continue[]
					];
					If[MatchQ[held, HoldComplete[Return[Run[_String]]]],
						processResult = ReleaseHold[
							held /. HoldComplete[Return[Run[command_String]]] :>
								HoldComplete[runCommand[command]]
						];
						If[processResult["StandardOutput"] =!= "",
							Print[StringTrim[processResult["StandardOutput"], "\n"]]
